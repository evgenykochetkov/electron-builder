...
class AppXTarget extends (_core || _load_core()).Target {
    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const tempDir = _path.join(_this.outDir, `__appx-temp-${(_builderUtil || _load_builderUtil()).Arch[arch]}`);
            yield (0, (_fsExtraP || _load_fsExtraP()).emptyDir)(tempDir);
            function getTempFile(name) {
                return _path.join(tempDir, name);
            }
            ...
            if (isScaledAssetsProvided(userAssets)) {
                ...
                const assetRoot = _path.join(tempDir, "appx/assets");
                ...
                for (const resourceFile of (yield (0, (_fsExtraP || _load_fsExtraP()).readdir)(tempDir)).filter(function (it) {
                    return it.startsWith("resources.");
                }).sort()) {
                    mappingList.push([`"${vm.toVmFile(_path.join(tempDir, resourceFile))}" "${resourceFile}"`]);
                }
                makeAppXArgs.push("/l");
            }
            let mapping = "[Files]";
